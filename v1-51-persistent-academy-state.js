...
const DEFAULT={activeScreen:"home",quickAccessId:null,vaultDay:null,soundEnabled:null,lastUpdated:null};
function load(){try{const p=JSON.parse(localStorage.getItem(KEY)||"null");return{...DEFAULT,...p,activeScreen:["home","academy","portfolio","video","support"].includes(p?.activeScreen)?p.activeScreen:"home",quickAccessId:typeof p?.quickAccessId==="string"?p.quickAccessId:null,vaultDay:Number.isInteger(Number(p?.vaultDay))&&Number(p.vaultDay)>=1&&Number(p.vaultDay)<=6?Number(p.vaultDay):null,soundEnabled:typeof p?.soundEnabled==="boolean"?p.soundEnabled:null,lastUpdated:typeof p?.lastUpdated==="string"?p.lastUpdated:null}}catch{return{...DEFAULT}}}
let state=load();
...
function restore(){if(location.hash==="#academy"){window.setTimeout(()=>{window.dispatchEvent(new CustomEvent("gei:navigation",{detail:{id:"academy",source:"v1-51-restore"}}));if(state.quickAccessId)window.dispatchEvent(new CustomEvent("gei:quick-access-restore",{detail:{id:state.quickAccessId}}));if(state.vaultDay)window.dispatchEvent(new CustomEvent("gei:vault-restore",{detail:{day:state.vaultDay}}));},0);return}if(state.quickAccessId||state.vaultDay)return;}
