...
function show(reason){
 if(visible&&panel.querySelector("pre")){
  panel.querySelector("pre").textContent=snapshot().join("\n");
  return;
 }
 visible=true;
 panel.hidden=false;
 ...
 title.textContent="Day 4 audio asset check";
 const message=document.createElement("div");
 message.textContent=reason||"The browser has not received playable audio metadata.";
 const pre=document.createElement("pre");
 pre.textContent=snapshot().join("\n");
...
function usable(){
 return audio.readyState>=HTMLMediaElement.HAVE_METADATA&&Number.isFinite(audio.duration)&&audio.duration>0;
}
function evaluate(){
 if(audio.error){show("The browser reported a media error.");return}
 if(usable()){hide();return}
 if(audio.networkState===HTMLMediaElement.NETWORK_NO_SOURCE){show("The browser reports that no usable media source was loaded.");return}
 show("The player is present, but no usable audio duration has been received yet.");
}
function arm(){
 window.clearTimeout(watchdog);
 watchdog=window.setTimeout(evaluate,4500);
}
...
audio.addEventListener("stalled",()=>arm());
audio.addEventListener("emptied",()=>arm());
window.addEventListener("pageshow",arm);
window.addEventListener("online",()=>{audio.load();arm()});
audio.preload="auto";
window.setTimeout(()=>{audio.load();arm()},0);
})();
