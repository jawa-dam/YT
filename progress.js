/* V1.10/V1.11/V1.12 — GEI Guided Entry, Progress & Learning Momentum */
(() => {
  "use strict";
  ...
  function updateJourney() {
    ...
    if (count === 0) {
      ...
      if (complete) { complete.textContent = "MARK DAY 1 COMPLETE"; complete.classList.remove("is-done"); } milestone?.classList.remove("is-visible");
    } else if (count < 6) {
      ...
      if (milestoneText) milestoneText.textContent = `${previous?.title.toUpperCase() || "DAY 1"} COMPLETE`; if (xp) xp.textContent = `+${count * 100} XP TOTAL`; milestone?.classList.add("is-visible");
    } else {
      ...
      if (milestoneText) milestoneText.textContent = "BLUEPRINT COMPLETE"; if (xp) xp.textContent = `+${state.xp} XP`; milestone?.classList.add("is-visible");
    }
