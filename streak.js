...
(() => {
  ...
  function dayGap(fromKey, toKey) {
    const from = dateFromKey(fromKey); const to = dateFromKey(toKey);
  ...
  function loadState() {
    try {
      ...
      return { streak: Math.max(0, Number(parsed.streak) || 0), best: Math.max(0, Number(parsed.best) || 0), lastActivityDate: typeof parsed.lastActivityDate === "string" ? parsed.lastActivityDate : null, milestone: Math.max(0, Number(parsed.milestone) || 0) };
    } catch (error) { return { ...DEFAULT_STATE }; }
  }
  function saveState() { try { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); } catch (error) {} }
  function getStatus() {
    const gap = state.lastActivityDate ? dayGap(state.lastActivityDate, localDateKey()) : null;
  ...
  function streakMarkup() {
    return `<section class="gei-streak-card" id="gei-streak-card" aria-labelledby="gei-streak-title"><div class="gei-streak-main"><span class="gei-streak-kicker">LEARNING MOMENTUM</span><h2 id="gei-streak-title">0 Day Streak</h2><p id="gei-streak-copy">Complete a GEI day today to start your learning rhythm.</p></div><div class="gei-streak-badge" aria-label="Current learning streak"><strong id="gei-streak-number">0</strong><span>DAYS</span></div><div class="gei-streak-meta"><span id="gei-streak-momentum">READY TO BUILD</span><span id="gei-streak-best">BEST 0 DAYS</span></div></section>`;
  }
  function ensureCard() {
  ...
  function render() {
    ensureCard();
    const card = document.getElementById("gei-streak-card"); if (!card) return;
    const today = localDateKey(); const activeToday = state.lastActivityDate === today;
    const number = card.querySelector("#gei-streak-number"); const title = card.querySelector("#gei-streak-title"); const copy = card.querySelector("#gei-streak-copy"); const momentum = card.querySelector("#gei-streak-momentum"); const best = card.querySelector("#gei-streak-best"); const status = getStatus();
    ...
    card.classList.toggle("is-active", activeToday); card.dataset.milestone = String(state.milestone || 0);
  }
  function updateAdam(detail) {
    const message = document.querySelector("#home-adam-assistant-message"); if (!message) return;
  ...
  function resetStreak() { state = { ...DEFAULT_STATE }; saveState(); render(); window.dispatchEvent(new CustomEvent("gei:streak-updated", { detail: { ...state, status: getStatus() } })); }
  function loadStyles() {
    if (document.querySelector('link[data-gei-streak-css]')) return;
    const link = document.createElement("link"); link.rel = "stylesheet"; link.href = "streak.css"; link.dataset.geiStreakCss = "true"; document.head.appendChild(link);
  }
  function init() {
    loadStyles(); render();
    window.addEventListener("gei:progress-updated", (event) => { if (event.detail?.completedDay) window.setTimeout(recordActivity, 0); });
